...
function swipeTo(direction, delta) {	
	let lastPosition = parseInt($wrapperContainer.get(0).style.marginLeft);
	if (!lastPosition) lastPosition = 0;
...
function scrolling(lastPosition, direction, delta) {
	let nextPosition = lastPosition + delta * direction;
...
function parallax(parallaxParent, parentShift) {
	let $parallaxChildren = $(parallaxParent).children("[data-parallax-role='child']");
	$parallaxChildren.each(function() {
		$(this).css("transform", `translate3d(${parentShift * $(this).attr("data-parallax-speed")}%, 0, 0)`);
	})
}
